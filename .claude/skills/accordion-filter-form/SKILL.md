...
<app-collapse-wrapper>
  <filter-form [dynamicFields]="filterfields()" (formSubmit)="onFormSubmit($event)"></filter-form>
</app-collapse-wrapper>
```
...
<app-collapse-wrapper>
  ...
  <filter-form #filterRef [dynamicFields]="filterfields()" (formSubmit)="onFormSubmit($event)"></filter-form>
</app-collapse-wrapper>
```
...
```html
<filter-form [dynamicFields]="filterfields()" (formSubmit)="onFormSubmit($event)" (controlValueChange)="onFilterChange($event)"></filter-form>
```
...
```typescript
export const myFilterFields = (tankerName: Option[] = [], plantName: Option[] = []): FieldConfig[] => [
  {
    name: "from",
    label: "From Date",
    type: "date",
...
```typescript
filterfields = computed<FieldConfig[]>(() => myFilterFields(this.tankerNameList(), this.plantNameList()));
...
## 11. API Service Patterns with forkJoin

### Using forkJoin for Multiple Parallel API Calls

When you need to call multiple APIs simultaneously (especially for populating filter dropdowns), use RxJS `forkJoin` for better performance:
...
export class MyService {
  ...
  // Combined method using forkJoin
...
```typescript
ngOnInit() {
  const filterParams = { GroupId: this.groupId, ForApp: '0' };
  const reportParams = { /* initial report params */ };

  this.myService.initializePageData(filterParams, reportParams).subscribe({
    next: (result) => {
      // Handle filter options
      this.mccList.set(result.filterOptions.mccData?.Data || []);
      this.milkTypeList.set(result.filterOptions.masterData?.MilkTypeList || []);

      // Handle report data
      this.reportData.set(result.reportData?.Data || []);
    },
    error: (error) => {
...
### Benefits of forkJoin Pattern

- **Performance**: Multiple APIs execute simultaneously instead of sequentially
- **Cleaner Code**: Single subscription handles all related data
- **Error Handling**: Centralized error handling for related API calls
- **Consistency**: All data arrives together, preventing UI flickering

### When to Use forkJoin

- ✅ Loading filter dropdown options (MCC, milk types, plants, etc.)
- ✅ Initializing page data that requires multiple API calls
- ✅ Related API calls that can execute independently
- ❌ Dependent API calls where one result affects another's parameters
