...
// import { commonServic } from './common.service'; // ✅ your existing CRUD wrapper
import { firstValueFrom } from 'rxjs';
import { AlertService } from '../../shared/services/alert.service';
import { MasterRequestService } from './master-request.service';
import { TripService } from './trip-dashboard/state-service/trip.service';
...
export class VehicleTrackingService {
  private commonService = inject(mapApiService);
  private mastersevice = inject(MasterRequestService);
  ...
  gropuid = localStorage.getItem('GroupId')!;
  ...
  async fetchVehicleTracking(req: VehicleTrackingRequest): Promise<any[]> {
    ...
    formData.append('group_id', this.gropuid || '');
    formData.append('AccountId', this.account_id || '');
    // formData.append('AccountId', req.vehicle_no || '');
    // formData.append('portal', req.portal || 'itraceit');
    formData.append('route_id', req?.route_id);
    try {
      ...
      if (
        ...
        response?.Result === 'Session Expired'
      ) {
        console.warn('Session Expired Detected!', response?.Message);

        // 1. User ko alert dikhayein (Agar aapke paas alert service hai)
        this.alertService.error(
          'Session Expired due to new login. Please login again.',
          'Error',
        );

        // 2. Local Storage / State clear karein aur Login page par redirect karein
        // this.storage.clear();
        // this.router.navigate(['/login']);

        // 3. Execution yahan rok dein aur caller ko error bhej dein taaki UI me purana data na dikhe
  ...
  //     formData.append('vehicle', req?.vehicle);

  //     try {
  //       const res: any = await firstValueFrom(
  //         this.commonService.distanceElockTimeline(formData),
  //       );
