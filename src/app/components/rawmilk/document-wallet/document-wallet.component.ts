import { Component, OnInit, signal, computed, ViewChild, TemplateRef, inject } from '@angular/core';
import { CommonModule } from '@angular/common';
import { forkJoin } from 'rxjs';
import { NgbModal } from '@ng-bootstrap/ng-bootstrap';
import { CollapseWrapperComponent } from '../../../shared/components/collapse-wrapper/collapse-wrapper.component';
import { FilterFormComponent, FieldConfig } from '../../../shared/components/filter-form/filter-form.component';
...
import { filterfields, vehicleDocumentColumns, actionColumn, editColumn, editVehicleDocumentFields } from './state-service/config';

@Component({
  selector: 'app-document-wallet',
  standalone: true,
  imports: [CommonModule, CollapseWrapperComponent, FilterFormComponent, SpkApexchartsComponent, AdvancedGridComponent],
...
export class DocumentWalletComponent implements OnInit {
  ...
  loadFilterOptions(): void {
    ...
    const indentPayload = createFormData(this.token, { GroupId: this.groupId, ForApp: '0' });
    ...
    forkJoin({
      ...
      indentMaster: this.documentWalletService.getCreateIndentMaster(indentPayload),
      ...
      next: (result) => {
        ...
        if (result.indentMaster?.PlantSupplier) {
          const suppliers = result.indentMaster.PlantSupplier.filter((item: any) => item.type == '6' || item.type === 6 || item.Type == '6' || item.Type === 6);
  ...
  fetchData(filterValues: any = {}): void {
    const payloadData = {
      ...
      VehicleId: filterValues?.vehicle?.VehicleId || filterValues?.vehicle?.id || '',
      DocTypeId: filterValues?.documentType?.DocumentTypeId || filterValues?.documentType?.id || '',
      Status: '',
      MpcId: filterValues?.supplier?.PlantId || filterValues?.supplier?.id || filterValues?.supplier?.SupplierId || '',
      TransporterId: filterValues?.transporter?.TransporterId || filterValues?.transporter?.id || ''
    };
    ...
    this.documentWalletService.getDocumentWalletRepo(payload).subscribe({
      next: (res: any) => {
        if (res && res.BarCount) {
          const { Active, Expired, ExpiredSoon } = res.BarCount;
        ...
        if (res && res.DocWalletVehicleData) {
          this.tableData.set(res.DocWalletVehicleData || []);
        } else {
          this.tableData.set([]);
        }
      },
  ...
  onEdit(data: any): void {
    const editFields = editVehicleDocumentFields(this.documentTypesList(), this.vehiclesList());
    
    // Attempt to map the initial options back based on data
    const initialVehicle = this.vehiclesList().find(v => v.VehicleNumber === data.VehicleNumber || v.VehicleNum === data.VehicleNumber);
    const initialDocType = this.documentTypesList().find(d => d.DocumentTypeName === data.DocumentTypeName);
      ...
      onSave: (formValues: any) => {
        return new Promise((resolve, reject) => {
          try {
            ...
            formData.append('DocumentTypeId', formValues?.documentType?.DocumentTypeId || initialDocType?.DocumentTypeId || "");
            formData.append('DocumentTypeName', formValues?.documentType?.DocumentTypeName || initialDocType?.DocumentTypeName || "");
            ...
            if (formValues?.uploadDocument && formValues.uploadDocument instanceof File) {
            ...
            formData.append('VehicleId', formValues?.vehicle?.VehicleId || initialVehicle?.VehicleId || data.VehicleId || "");
            ...
            this.documentWalletService.editDocument(formData).subscribe({
              next: (res: any) => {
                if (res.Status === 'success') {
                  this.alertService.success(res.Message || 'Document updated successfully.');
                  this.fetchData(); // Refresh grid
                  resolve(true);
                } else {
                  this.alertService.error(res.Message || 'Failed to update document.');
                  reject(res.Message);
                }
              },
              error: (err: any) => {
                this.alertService.error(err?.message || 'Error updating document');
                reject(err);
              }
            });
          } catch (error: any) {
