...
export class MarkerConfigService {
 public getMarkerConfig(marker: MarkerData , vehicleStatus: string = '') {
    switch (marker.markerType) {
      ...
      case 'alert':
        return { icon: 'assets/images/users/icons-flag-big.png', category: 'alert', zIndex: 1 };
      case 'customer':
   ...
   private getTrackingConfig(marker: MarkerData, vehicleStatus: string) {
    ...
    if (marker.isStart) {
        return { icon: 'assets/imagesnew/start_marker.png', category: 'static', zIndex: 100 };
    }
    ...
    if (marker.isEnd) {
        ...
        if (isVehicleInactive(vehicleStatus)) {
            return { icon: 'assets/imagesnew/stop_marker.png', category: 'static', zIndex: 100 };
        }
    ...
    const speed = marker.params?.['speed'] ?? marker.params?.['speed(Km/hr)'] ?? 0;
    return this.getSpeedBasedConfig(speed);
  }
  
  private getSpeedBasedConfig(speed: number) {
    if (speed > 20) return { icon: 'assets/imagesnew/green_Marker1.png', category: 'green', zIndex: 100 };
    if (speed > 5) return { icon: 'assets/imagesnew/yellow_Marker1.png', category: 'yellow', zIndex: 100 };
    return { icon: 'assets/imagesnew/red_Marker1.png', category: 'red', zIndex: 100 };
  }
}