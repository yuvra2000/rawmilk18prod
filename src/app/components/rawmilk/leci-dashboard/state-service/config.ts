...
function lockStatusRenderer(locks: Array<Record<string, string>>): string {
    if (!locks || locks.length === 0) return '<span style="color:#aaa">-</span>';
    return locks.map(lockObj => {
        const [key, val] = Object.entries(lockObj)[0] || [];
        if (!key) return '';
        const isLocked = val === 'Y';
        const icon = isLocked
...
export const alertDetailColumns: GridColumnConfig[] = [
    ...
    },
    {
        ...
        cellRenderer: (params: any) => {
            ...
            const colorMap: Record<string, string> = {
                S20: '#856404', S30: '#721c24',
                'LID TAMPERING': '#004085', 'TANKER HOLD': '#5a2d82',
            };
            const bgMap: Record<string, string> = {
                S20: '#fff3cd', S30: '#f8d7da',
                'LID TAMPERING': '#cce5ff', 'TANKER HOLD': '#e8d5f5',
            };
    ...
    {
        headerName: 'Dispatch No',
        valueGetter: (params: any) => params.data?.shipment_no || params.data?.primary_info?.shipment_no || '-',
        minWidth: 140,
    },
    {
        headerName: 'Dispatch Date',
        valueGetter: (params: any) => params.data?.run_date ? params.data.run_date.split(' ')[0] : '-',
        minWidth: 130,
    },
    {
        headerName: 'Plant',
        valueGetter: (params: any) => {
            const code = params.data?.primary_info?.destination_code || params.data?.destination_code || '';
    ...
    {
        headerName: 'MPC',
        valueGetter: (params: any) => params.data?.supplier_name || params.data?.primary_info?.driver_name || '-',
        minWidth: 160,
    },
    {
        headerName: 'MCC',
        valueGetter: (params: any) => {
            const code = params.data?.primary_info?.source_code || params.data?.source_code || '';
